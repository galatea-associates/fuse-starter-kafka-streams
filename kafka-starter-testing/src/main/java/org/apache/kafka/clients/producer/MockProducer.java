...
public class MockProducer<K, V> implements Producer<K, V> {
   ...
   * @param autoComplete If true automatically complete all requests successfully and execute the
   * callback. Otherwise the user must call {@link #completeNext()} or {@link
   * #errorNext(RuntimeException)} after {@link #send(ProducerRecord) send()} to complete the call
   * and unblock the {@link java.util.concurrent.Future Future&lt;RecordMetadata&gt;} that is
   * returned.
  ...
  public MockProducer(final boolean autoComplete,
      ...
      final Serializer<K> keySerializer,
      final Serializer<V> valueSerializer) {
    this(MockCluster.empty(), autoComplete, partitioner, keySerializer, valueSerializer);
  }
  ...
  public void sendOffsetsToTransaction(Map<TopicPartition, OffsetAndMetadata> offsets,
    ...
    Map<TopicPartition, OffsetAndMetadata> uncommittedOffsets = this.uncommittedConsumerGroupOffsets
        .get(consumerGroupId);
  ...
  /**
   * Adds the record to the list of sent records. The {@link RecordMetadata} returned will be
  ...
  public synchronized Future<RecordMetadata> send(ProducerRecord<K, V> record, Callback callback) {
    ...
    // recreate the record using all original values except using the calculated partition
    record = new ProducerRecord<>(record.topic(), partition, record.timestamp(), record.key(),
   ...
   */
  private int partition(ProducerRecord<K, V> record, MockCluster cluster) {
    ...
    return this.partitioner
        .partition(topic, record.key(), keyBytes, record.value(), valueBytes,
